...
TEST_CASE("Test Board API", "[IMU]") {
    ...
    TEST_LIMIT(150, 300, bsp_cmd_read(BSP_DRIVER_FIRMWARE, 0));
    ...
    for (int i=0; i<sizeof(read_only)-2; i++) {
    ...
    TEST_ERROR(ESP_ERR_INVALID_SIZE, bsp_cmd_write(BSP_SERVO_CONFIG_PERIOD, 0, 0xFFFF+1));
    // Test port validation
    TEST_ERROR(ESP_ERR_INVALID_ARG, bsp_cmd_write(BSP_DC_POWER, 4, 0));
    TEST_ERROR(ESP_ERR_INVALID_ARG, bsp_cmd_write(BSP_SERVO_PULSE, 3, 0));
    TEST_ERROR(ESP_ERR_INVALID_ARG, bsp_cmd_write(BSP_RGB_COLOR, 4, 0));
    // Test cmd validation
