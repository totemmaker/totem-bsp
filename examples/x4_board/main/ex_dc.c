...
/**
 * RoboBoard X4 has 4 individual motor ports for connecting brushed motors.
...
void run_example_dc(void) {
    ...
    // Control motors
    // Default A,B,C,D port settings:
    // Older driver versions: fast decay, 50Hz
    // Version 1.60 and later: slow decay, 20kHz
    ESP_LOGI("Board", "--- Spin motors ---");
    // Interact with motors
    while (1) {
        // Play 45kHz tone on all motors
        ESP_LOGI("Motor", "Tone 4000Hz");
        bsp_dc_tone_ABCD(4000, 0);
